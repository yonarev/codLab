...
setInterval(cambiarColor, 40000);
// iniciar()
function iniciar(){
...
function grabaLocal(opcion){
    ...
    stopTimer()
    showTime()
    //let preguntaValidar=confirm("Desea Validar los datos ingresados")
...
function agregarRecordatorio(event) {
    ...
    if(id==='' || nombre==='' || gestion===''){
      alert ('no hay datos recordatorio')
      document.getElementById('recordatorio').select()
      document.getElementById('recordatorio').focus()
      return
    }
...
eliminarBtn.addEventListener('click', eliminarRecordatorios);
...
var activarRadio = document.getElementById("activar-select");
var desactivarRadio = document.getElementById("desactivar-select");
var miSelect = document.getElementById("horaIni");
//habilita select
activarRadio.addEventListener("change", function() {
      if (activarRadio.checked) {
        miSelect.disabled = false; // habilitar el select
      }
});
//desactiva seleccion
desactivarRadio.addEventListener("change", function() {
      if (desactivarRadio.checked) {
        miSelect.disabled = true; // deshabilitar el select
      }
});
...
//   //tiempo.innerHTML = `${diasTranscurridos} días, ${horasTranscurridas % 24} horas, ${minutosTranscurridos % 60} minutos y ${segundosTranscurridos % 60} segundos`;
//   tempo.innerHTML = `${minutosTranscurridos % 60} minutos y ${segundosTranscurridos % 60} segundos`;
...
window.onload = function() {
  ...
  fijaAsunto();
  // Agrega el evento click al botón Set Asunto
